define([
    ...
    return Column.extend({
        ...
        getLabel: function (row) {
            ...
            if (validation) {
                var data = this.tryParseJSON(validation);
                if (data) {
                    var counter = 0;
        ...
        startView: function (row) {
            if (this.getProductvalidation(row)) {
                var previewPopup = $('<div></div>',{id : 'cdiscountpopup'+this.getProductId(row) });
                ...
                var cdiscountpopup = previewPopup.modal({
                    title: this.getTitle(row),
                    innerScroll: true,
                    modalLeftMargin: 15,
                    buttons: [],
                    opened: function (row) {
                        cdiscountpopup.append(result);
                    },
                    closed: function (row) { }
                }).trigger('openModal');
            }
