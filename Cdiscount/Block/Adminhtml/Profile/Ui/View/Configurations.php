...
class Configurations extends \Magento\Config\Block\System\Config\Form
{
    ...
    protected function _initGroup(
        ...
        $_frontendModelClass = $group->getFrontendModel();
        $fieldsetRenderer = $_frontendModelClass ? $this->_layout->getBlockSingleton(
            $_frontendModelClass
        ...
        $_fieldset = $form->addFieldset($this->_generateElementId($group->getPath()), $_fieldsetConfig);
        $_fieldset->setRenderer($fieldsetRenderer);
        $group->populateFieldset($_fieldset);
        $this->_addElementTypes($_fieldset);
        ...
        if ($group->shouldCloneFields()) {
            ...
            foreach ($cloneModel->getPrefixes() as $prefix) {
                $this->initFields($_fieldset, $group, $section, $prefix['field'], $prefix['label']);
            }
        } else {
            $this->initFields($_fieldset, $group, $section);
        }

        $this->_fieldsets[$group->getId()] = $_fieldset;
    }
    ...
    public function initFields(
        ...
        /** @var $item \Magento\Config\Model\Config\Structure\Element\Field */
        foreach ($group->getChildren() as $item) {
            if ($item instanceof \Magento\Config\Model\Config\Structure\Element\Group) {
                $this->_initGroup($item, $section, $fieldset);
            } else {
                $path = $item->getConfigPath() ?: $item->getPath($fieldPrefix);
                if ($item->getSectionId() != $section->getId()) {
                    $groupPath = $item->getGroupPath();
                ...
                $this->_initElement($item, $fieldset, $path, $fieldPrefix, $labelPrefix);
            }
        }
        return $this;
    }
    ...
    protected function _initElement(
        ...
        $formField = $fieldset->addField(
            ...
            [
                ...
                'data-form-part' => "cdiscount_profile_edit", // added for for form save to send values
            ]
        );
