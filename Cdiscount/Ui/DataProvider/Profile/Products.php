...
class Products extends \Magento\Ui\DataProvider\AbstractDataProvider
{
    ...
    public function __construct(
        ...
        $profileId = $this->request->getParam('cdiscount_profile_id');
        $bookmarks = $this->bookmark->create()
            ->getCollection()
            ->addFieldToFilter('namespace', ['eq' => 'cdiscount_products_index']);
        if (isset($profileId) and !empty($profileId)) {
            foreach ($bookmarks as $bookmark) {
                if ($bookmark->getIdentifier() == 'current') {
                    $configValue = $bookmark->getConfig();
                    $configValue['current']['filters']['applied']['cdiscount_profile_id'] = $profileId;
            ...
            foreach ($bookmarks as $bookmark) {
                if ($bookmark->getIdentifier() == 'current') {
                    $configValue = $bookmark->getConfig();
                    if (isset($configValue['current']['filters']['applied']['cdiscount_profile_id'])) {
                        unset($configValue['current']['filters']['applied']['cdiscount_profile_id']);
                    }
                    $bookmark->setConfig(json_encode($configValue));
                    $bookmark->save();
                }
     ...
     * AddFilter
     *
     * @param \Magento\Framework\Api\Filter $_filter
     * @return void
     */
    public function addFilter(\Magento\Framework\Api\Filter $_filter)
    {
        if (isset($this->addFilterStrategies[$_filter->getField()])) {
            $this->addFilterStrategies[$_filter->getField()]
                ->addFilter(
                    $this->getCollection(),
                    $_filter->getField(),
                    [$_filter->getConditionType() => $_filter->getValue()]
                );
        } else {
            parent::addFilter($_filter);
        }
    }
