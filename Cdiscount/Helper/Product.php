...
class Product extends \Magento\Framework\App\Helper\AbstractHelper
{
     ...
     * @param \Sdk\Offer\OfferFilter $offerFilter
     * @param \Sdk\ApiClient\CDSApiClientFactory $apiClient
     * @param \Sdk\Product\IdentifierRequest $identifierRequest
     * @param \Ced\Cdiscount\Model\FeedsFactory $feedsFactory
     * @param \Ced\Cdiscount\Model\ResourceModel\Category\CollectionFactory $categoriesFactory
     * @param BarcodeValidator $barcodeValidator
     * @param Config $config
     * @param Logger $logger
     * @param Profile $profile
     * @param CollectionFactory $attributeCollectionFactory
     * @param \Ced\Cdiscount\Model\Xml\GeneratorFactory $generatorFactory
     * @param Html2TextFactory $html2TextFactory
     */
    public function __construct(
        ...
        \Sdk\Offer\OfferFilter $offerFilter,
    ...
    public function getUploadedProductStatus($ids = [])
    {
        try {
            ...
            if (isset($ids) and !empty($ids)) {
                $skus = [];
                $mappedAttributeForSellerProductId = false;
                ...
                foreach ($ids as $id) {
                    ...
                    if ($products->getTypeId() == 'configurable' &&
                        ...
                        foreach ($childs as $product) {
                            $mappedAttributeForSellerProductId = $this->profileHelper->getProfile($id, $profileId)
                                ->getMappedAttribute(
                                    \Ced\Cdiscount\Helper\Category::ATTRIBUTE_SELLER_PRODUCT_ID
                                );

                            if ($mappedAttributeForSellerProductId != false &&
                                !empty($product->getData($mappedAttributeForSellerProductId))) {
                                $sku = trim((string)$product->getData($mappedAttributeForSellerProductId));
                                $skus[] = $sku;
                            }
                        }
                        ...
                        $products->getVisibility() != 1) {
                        $mappedAttributeForSellerProductId = $this->profileHelper->getProfile($id, $profileId)
                            ->getMappedAttribute(\Ced\Cdiscount\Helper\Category::ATTRIBUTE_SELLER_PRODUCT_ID);
                        if ($mappedAttributeForSellerProductId != false &&
                            !empty($products->getData($mappedAttributeForSellerProductId))) {
                            $skus[] = $sku = trim((string)$products->getData($mappedAttributeForSellerProductId));
                        }
                    }
                }
                $skusToSync = [];
                ...
                $offerFilter->setSellerproductIds($skus);
                // issue is here because opf the sdk file
                $offerResponse = $client->getOfferPoint()->getOfferListPaginated($offerFilter, 1);
                /** @var \Sdk\Offer\Offer $offer */
                if ($offerResponse->getOfferList()) {
                    foreach ($offerResponse->getOfferList() as $offer) {
                        $skusToSync[$offer->getSellerProductId()] = $offer->getSellerProductId();
                    }
                }

                if (!empty($mappedAttributeForSellerProductId) && isset($skusToSync) && !empty($skusToSync)) {
                    $magentoProductsCollection = $this->collectionFactory->create()
                        ->addAttributeToFilter($mappedAttributeForSellerProductId, ['in' => array_merge($skusToSync, $parentSkus)]);
                    if (!empty($magentoProductsCollection->getAllIds())) {
                        $updateIds = $magentoProductsCollection->getAllIds();
                        $parentIds = $this->configurableFactory->create()
                            ->getParentIdsByChild($updateIds);
                        if (isset($parentIds) && !empty($parentIds)) {
                            $parentIds = array_unique($parentIds);
                            $updateIds = array_merge($updateIds, $parentIds);
                        }
                        $this->action->updateAttributes($updateIds,
                            [self::CDISCOUNT_PRODUCT_STATUS => \Ced\Cdiscount\Model\Source\Product\Status::LIVE],
...
//            echo '<pre>' ; print_r($e->getMessage());  die('<-- catch');
            $response = false;
            if ($this->debugMode == true) {
    ...
    public function createProductPackage($data)
    {
        try {
            ...
            if (isset($data)) {
                ...
                $response = $this->sendProductPackage('Product_' . $timestamp . ".zip", $timestamp);
            }
            return $response;
        } catch (\Exception $e) {
            if ($this->debugMode == true) {
    ...
    public function createOfferPackage($data)
    {
        try {
            ...
            if (isset($data)) {
                ...
                $dir = $this->createDir($path);
                $name = 'Offers.xml';
                $directoryName = 'Offers'. $timestamp;
//                $name = 'Offers.text';
                $path = $dir['path'] . DS . $name;
                $this->offerPath = $path;
                $product = $this->generator->create()->arrayToXml($data);
    ...
    public function sendProductPackage($name, $idNameForFailed = false)
    {
        try {
            $response = true;
            $userName = $this->config->getUserName();
            ...
            if (!empty($token)) {
                ...
                if ($packageId) {
                    ...
                    $this->saveFailedResponse($url, $submitProductPackageResponse->getErrorMessage(), $name, $idNameForFailed);
...
//                        }
                    ...
                    if ($cdiscountAttributeId == \Ced\Cdiscount\Helper\Category::ATTRIBUTE_EAN &&
                        ...
                        if(is_bool($ean)){
                            $errors["$cdiscountAttributeId"] = "Ean contain Speacial Character";
                        }elseif (!$ean->isValid()) {
                ...
                if (!empty($errors)) {
                    if(!is_array($validatedProduct)) $validatedProduct = [];
            ...
            foreach ($ids as $id) {
                ...
                if (isset($optionalAttributes) and !empty($optionalAttributes)) {
                    ...
                    foreach ($optionalAttributes as $attributeName => $optionalValue) {
                        if (!in_array($attributeName, [Category::ATTRIBUTE_SIZE, Category::ATTRIBUTE_SELLER_PRODUCT_COLOR_NAME])) {
    ...
    public function getPrice($productObject)
    {
        ...
        $splprice = $this->format->getNumber($splprice);
        $price = $this->format->getNumber($price);
    ...
    public function saveFailedResponse($url, $failedResponse, $name, $idNameForFailed) {
        ...
        $feedModel->addData([
            'feed_id' => 'empty_'.$idNameForFailed,
    ...
    public function saveResponse($response, $type, $name)
    {
        ...
        if($type == "offer_creation") {
            $nav = explode(".", $name);
            $this->packageName = $nav[0];
            $packageId = $packageId . "_" . $nav[0];
        }
        try {
                            ...
                            foreach ($products as $product) {
                                ...
                                if ($this->config->getPriceMapping() &&
                                    ...
                                    [$this->key]['Offer']['_attribute']['Price'] =
                                        (string)$this->format->getNumber($product->getData($this->config->getPriceMapping()));
                                } else {
            ...
            foreach ($ids as $id) {
                ...
                if ($product->getTypeId() == 'configurable' &&
                    ...
                    foreach ($products as $product) {
                        ...
                        $salableObj = $objectManager->create('Magento\InventorySalesAdminUi\Model\GetSalableQuantityDataBySku');
                        $salableQuantitys = $salableObj->execute($product->getSku());
                        $stock = 0;
                        if (isset($salableQuantitys)) {
                            foreach ($salableQuantitys as $salableQuantity) {
                                $stock = $stock + $salableQuantity['qty'];
                            }
                        }
...
//                    $stock = $this->stockState->getStockQty($product->getId(), $product->getStore()->getWebsiteId());
                    ...
                    if (isset($requiredAttributes[Category::ATTRIBUTE_EAN]
                            [self::MAGENTO_ATTRIBUTE_CODE]) && !empty($requiredAttributes[Category::ATTRIBUTE_EAN]
                        [self::MAGENTO_ATTRIBUTE_CODE])) {

                        $this->offer['OfferPackage']['_value']['OfferPackage.Offers']
                        ['_value']['OfferCollection']['_value'][$this->key]['Offer']['_attribute']
                        ['ProductEan'] = $product
                            ->getData($requiredAttributes[Category::ATTRIBUTE_EAN]
                            [self::MAGENTO_ATTRIBUTE_CODE]);
                    ...
                    foreach ($optionalAttributes as $optionalAttributeKey => $optionalAttributeValue) {
                        if ($optionalAttributeKey == Category::ATTRIBUTE_COMMENT) {
                        ...
                        if ($optionalAttributeKey == Category::ATTRIBUTE_SALE_PRICE) {
                            if (!empty($product->getData($optionalAttributeValue[self::MAGENTO_ATTRIBUTE_CODE]))) {
                                $this->offer['OfferPackage']['_value']
                                ['OfferPackage.Offers']['_value']['OfferCollection']['_value']
                                [$this->key]['Offer']['_attribute']['Price'] =
...
//            echo '<pre>' ;  print_r($this->offer);  die('<--');
            $this->createOfferPackage($this->offer);
            $response = true;
        }
        return $response;
