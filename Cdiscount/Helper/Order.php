...
class Order extends \Magento\Framework\App\Helper\AbstractHelper
{
    ...
    public function __construct(
        ...
        $this->shipmentMethod = $config->getShipmentMethod();
    }
    ...
    public function importOrders()
    {
        try {
            ...
            $current_date = $this->getCurrentDateTime();            
            $creationDate = $this->getDateTime15DaysAgo();
            $orderFilter->setBeginCreationDate($creationDate);
            $orderFilter->setBeginModificationDate($creationDate);
            $orderFilter->setEndCreationDate($current_date);
            $orderFilter->setEndModificationDate($current_date);
            ...
            if (isset($orders)) {
                foreach ($orders as $order) {
                    ...
                    if (!$this->validateString($cdiscountOrder->getData())) {
                        $customer = $this->getCustomer($order, $websiteId, $store);
                        if ($customer !== false) {
    ...
    public function getDateTime15DaysAgo()
    {
        $currentDateTime = $this->dateTime->gmtDate();
        $timestamp = strtotime($currentDateTime);
        $dateTime15DaysAgo = date('Y-m-d\TH:i:s', strtotime('-15 days', $timestamp));
        $dateTime15DaysAgo = str_replace(' ', 'T', $dateTime15DaysAgo);
        $dateTime15DaysAgo .= '.00';    

        return $dateTime15DaysAgo;
    }
     ...
     */
    public function getCustomer($order, $websiteId, $store)
    {
...
//                    $customer->setWebsiteId($websiteId);
                    ...
                    $this->logger->log(
                        ...
                        $order->getOrderNumber() . ' Message:' . $e->getMessage()
                    );
                    return false;
                }
            }
            ...
            if (isset($items)) {
                foreach ($items as $item) {
                    ...
                    if (isset($sku) && empty($sku)) {
                        if ($item->getProductId() == "INTERETBCA") {
                            $sku = $this->config->getAdditionalFeeSku();
                            if ($sku == "") {
                                continue;
                            }
                        } else {
...
//                                         }
                                        ...
                                        //Customization getting price from Magento
                ...
                if ($itemAccepted > 0) {
                    try {
                        ...
                        $shippingAddress->setCollectShippingRates(true)->collectShippingRates()
                            ->setShippingMethod($this->shipmentMethod);
                        ...
                        $orderData = [
                            ...
                            'order_data' => \Magento\Framework\Serialize\SerializerInterface::serialize($order),
                            'order_items' => \Magento\Framework\Serialize\SerializerInterface::serialize($items)
                        ];
        ...
        {
            $items = $order->getOrderLineList()->getOrderLines();
            $rea[] = $exception->getMessage();
            $this->rejectorder($order,$items,$rea);
    ...
    public function acknowledgeOrders($order)
    {
        ...
        if (isset($ordersData) and !empty($ordersData)) {
            $orderItems = (\Magento\Framework\Serialize\SerializerInterface::unserialize($ordersData[0]['order_items']));
            $username = $this->config->getUserName();
            $password = $this->config->getUserPassword();
...
//        mail($to_email, $subject, $body, $headers);
    ...
    public function shipOrder(array $data = [], $fromObserver = false)
    {
        ...
        try {
            ...
            $cdiscountOrder = \Magento\Framework\Serialize\SerializerInterface::unserialize($cdsicountOrderData->getData('order_items'));
            if ($cdiscountOrder) {
    ...
    public function processOrderItems($order)
    {
        ...
        $cdiscountOrderItemsData = \Magento\Framework\Serialize\SerializerInterface::unserialize($order->getOrderItems()); // update
